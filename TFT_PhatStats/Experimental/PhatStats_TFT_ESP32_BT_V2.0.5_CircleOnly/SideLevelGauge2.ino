...
void CPU_tempLevelGauge(int cpuDegree ) {
  ...
  tft.fillRect (294 - 6 , 85 + 10,  25, 18 , ILI9341_BLACK); // Stop Scale running into MAX box
...
void GPU_tempLevelGauge(int gpuDegree ) {
  ...
  tft.drawRoundRect(299 - 6 , 129,   8, 90 , 1, ILI9341_WHITE);  // Draw rectangle round GPU temp bar
  
  /*Stop Scale running into MAX box,*/
  tft.fillRect (294 + 2 - 6 , 219,  25, 17 , ILI9341_BLACK);
...
void GPU_loadLevelGauge(int gpuUsage ) {
  ...
  if (GPUloadfactor >= 90 ) {
    ...
    tft.drawRoundRect(311 - 5 , 129,   8, 90 , 1, ILI9341_WHITE); // Draw rectangle round GPU load bar
}
